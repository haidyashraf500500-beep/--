...
def build_grounded_prompt(query: str, context_text: str) -> str:
    return f"""
أنت مساعد حكومي دقيق ومحايد.

القواعد:
- استخدم فقط المعلومات الموجودة في السياق المرفق.
- إذا لم تجد الإجابة داخل السياق، قل:
"عذراً، لا أعرف الإجابة بناءً على المستندات المتاحة."
- لا تستخدم معلومات خارجية.
- لا تخترع إجابات أو مصادر.
- اذكر المصادر فقط إذا كانت الإجابة مبنية على السياق.
